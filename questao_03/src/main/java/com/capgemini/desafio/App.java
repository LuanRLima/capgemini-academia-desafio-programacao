...
public class App {
    ...
    public static void main(String[] args) {
		int anagrama =  quantidadeAnagramas("ovo");
    ...
    /** 
     * Método que indetifica a quantidade de anagramas de uma palavra de si mesma, dada uma string qualquer, encontrar  o número de pares de substrings que são anagramas.
    ...
    public static int quantidadeAnagramas(String s) {
        ...
        for(int i = 0; i < lista.size(); i++) {
            for(int j = i + 1; j < lista.size(); j++) {
                if(validacoesAnagramas(lista.get(i), lista.get(j))) resultado++;
            }
        }
        return resultado;
    }
